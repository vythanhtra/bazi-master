...
const ALLOW_LEGACY_TOKENS = import.meta.env.MODE === 'test' || import.meta.env.VITE_E2E === '1';
...
const parseUserParam = (value: string | null) => {
...
const persistLegacyAuth = (token: string, user: Record<string, unknown> | null) => {
  if (!ALLOW_LEGACY_TOKENS || !token) return;
  try {
  ...
  try {
    sessionStorage.setItem('bazi_token', token);
    sessionStorage.setItem('bazi_token_origin', 'backend');
    if (user) {
      sessionStorage.setItem('bazi_user', JSON.stringify(user));
    }
  } catch {
    // Ignore storage failures.
  }
...
export default function Login() {
  ...
  const sanitizeNextPath = (value: unknown): string | null => sanitizeRedirectPath(value, null);

  const getStateRedirectPath = useCallback(() => {
  ...
  const buildAuthSearch = useCallback(() => {
  ...
  }, [getStateRedirectPath, location.search]);

  const resolveRedirectPath = useCallback((from: unknown, search = '') => {
    if (from) {
      if (typeof from === 'string') return sanitizeNextPath(from) || '/profile';
      const f = from as { pathname?: string; search?: string; hash?: string };
      if (f.pathname) {
        return sanitizeNextPath(`${f.pathname}${f.search || ''}${f.hash || ''}`) || '/profile';
      }
    }
    if (search) {
  ...
  const getLoginErrorMessage = (error: unknown) => {
    const message =
      error instanceof Error && error.message ? error.message : t('login.errors.loginFailed');
  ...
  }, [isAuthenticated, location.search, location.state, navigate, resolveRedirectPath]);
  ...
  }, [buildAuthSearch, location.search, location.state, navigate, t]);
  ...
  useEffect(() => {
    ...
    const nextPath =
      sanitizeNextPath(nextParam) || resolveRedirectPath(location.state?.from, location.search);

    if (tokenParam && ALLOW_LEGACY_TOKENS) {
      const parsedUser = parseUserParam(userParam);
      persistLegacyAuth(tokenParam, parsedUser);
      refreshUser()
        .catch(() => {})
        .finally(() => {
  ...
  }, [location.search, location.state, navigate, refreshUser, resolveRedirectPath]);
  ...
  const handleRegister = async (event: FormEvent) => {
    ...
    if (registerConfirm && registerConfirm !== registerPassword)
      nextErrors.confirm = t('login.errors.passwordsNotMatch');
    if (registerName.trim().length > 0 && registerName.trim().length < 2)
  ...
  const handleRequestReset = async (event: FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        setResetStatus({
          type: 'error',
          message: await readApiErrorMessage(res, t('login.errors.resetRequestFailed')),
        });
        return;
      }
  ...
  const handleConfirmReset = async (event: FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        setResetStatus({
          type: 'error',
          message: await readApiErrorMessage(res, t('login.errors.resetPasswordFailed')),
        });
        return;
      }
  ...
  return (
    <main
      id="main-content"
      tabIndex={-1}
      className="responsive-container flex min-h-[70vh] flex-col items-center justify-center gap-4"
    >
