...
export const generateSoulPortrait = async (req, res, next) => {
  try {
    ...
    const { openaiApiKey, aiProvider } = getServerConfig();
    const imageProvider = openaiApiKey ? 'openai' : 'mock';
    if (!openaiApiKey && aiProvider !== 'mock') {
      logger.warn({ aiProvider }, 'OpenAI not configured; using mock image provider');
    }
    const imageUrl = await generateImage({ prompt, provider: imageProvider });
