    ...
    <section className="mt-8 rounded-3xl border border-indigo-500/30 bg-indigo-900/20 p-6">
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="text-sm text-white/60">{t('tarot.weeklySnapshotSubtitle')}</p>
        </div>
        <Button onClick={onFetch} disabled={loading} isLoading={loading} variant="secondary">
        ...
        <div className="mt-6 rounded-2xl border border-white/10 bg-white/5 p-6 text-white/80">
          <div className="flex flex-col gap-1">
            <h3 className="font-display text-xl text-white">
              {t(`zodiac.signs.${horoscope.sign.name.toLowerCase()}`)}{' '}
              {t(`zodiac.periods.${horoscope.period}`)} {t('tarot.horoscope')}
            </h3>
            <p className="text-xs text-white/60">
              {horoscope.range} • {t(`zodiac.ranges.${horoscope.sign.name.toLowerCase()}`)} •{' '}
              {t('zodiac.generatedAt', { date: new Date(horoscope.generatedAt).toLocaleString() })}
            </p>
          </div>
