  ...
  useEffect(() => {
    const fetchDaily = async () => {
      try {
        ...
        logger.error({ e }, 'Failed to load daily fortune');
      } finally {
        setLoading(false);
      }
    };

    if (isAuthenticated) fetchDaily();
      ...
      <div className="flex justify-between items-start mb-4">
        <div>
          <h3 className="text-gold-400 font-display text-lg">
            {t('calendar.today', "Today's Energy")}
          </h3>
          <div className="text-white/60 text-sm">{dailyPillar.date}</div>
        </div>
        <div className="text-right">
          <div className="text-2xl font-bold text-white">
            {dailyPillar.charStem}
            {dailyPillar.charBranch}
          </div>
      ...
      <div className="border-t border-white/10 pt-4 mt-2">
        {fortune.score ? (
          ...
          <p className="text-sm text-white/50">
            {t('calendar.noData', 'Update profile for personalized insights.')}
          </p>
        )}
      </div>
    </Card>
