...
export default function Synastry() {
  ...
  return (
    <div className="container mx-auto p-6 max-w-4xl">
      <h1 className="text-3xl font-display text-gold-400 mb-8 text-center">
        {t('synastry.title', 'Relationship Compatibility')}
      </h1>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-3 gap-2">
              <input
                ...
                onChange={(e) => handleChange('A', 'birthYear', e.target.value)}
              />
              <input
                type="number"
                className="bg-white/10 p-2 rounded text-white"
                ...
                onChange={(e) => handleChange('A', 'birthMonth', e.target.value)}
              />
              <input
                type="number"
                className="bg-white/10 p-2 rounded text-white"
                ...
                onChange={(e) => handleChange('A', 'birthDay', e.target.value)}
              />
            </div>
            <select
              className="w-full bg-white/10 p-2 rounded text-white"
              value={personA.gender}
              onChange={(e) => handleChange('A', 'gender', e.target.value)}
            >
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-3 gap-2">
              <input
                ...
                onChange={(e) => handleChange('B', 'birthYear', e.target.value)}
              />
              <input
                type="number"
                className="bg-white/10 p-2 rounded text-white"
                ...
                onChange={(e) => handleChange('B', 'birthMonth', e.target.value)}
              />
              <input
                type="number"
                className="bg-white/10 p-2 rounded text-white"
                ...
                onChange={(e) => handleChange('B', 'birthDay', e.target.value)}
              />
            </div>
            <select
              className="w-full bg-white/10 p-2 rounded text-white"
              value={personB.gender}
              onChange={(e) => handleChange('B', 'gender', e.target.value)}
            >
              <option value="female">Female</option>
              <option value="male">Male</option>
            </select>
      ...
      {result && (
        <Card className="p-8 bg-black/40 backdrop-blur-xl border-gold-500/30 animate-fade-in">
          ...
          <div className="grid md:grid-cols-2 gap-8">
            <div>
              <h4 className="text-gold-400 border-b border-white/10 pb-2 mb-4">
                Profile Comparison
              </h4>
              <p className="text-white mb-2">
                <span className="text-white/50">{result.personA.name}:</span>{' '}
                {result.personA.element} {result.personA.dayMaster}
              </p>
              <p className="text-white mb-2">
                <span className="text-white/50">{result.personB.name}:</span>{' '}
                {result.personB.element} {result.personB.dayMaster}
              </p>
            </div>
            <div>
