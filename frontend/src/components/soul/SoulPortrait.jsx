...
export default function SoulPortrait() {
  ...
  const { isAuthenticated, user } = useAuth(); // We might need user bazi data here or pass it
  ...
  const generatePortrait = async () => {
    if (!isAuthenticated) {
      setError('Please sign in to generate a portrait.');
      return;
    }
    setLoading(true);
    setError(null);
    try {
      ...
      logger.error({ err }, 'Soul portrait generation failed');
    ...
    <div className="container mx-auto max-w-4xl p-6">
      ...
      <Card className="text-center p-8 bg-black/40 backdrop-blur-xl border-white/10">
        <h1 className="text-3xl font-display text-gold-400 mb-4">
          {t('soul.title', 'Soul Portrait')}
        </h1>
        <p className="text-white/70 mb-8 max-w-lg mx-auto">
          {t(
            'soul.description',
            'Visualize the essence of your BaZi chart through AI-generated art. Each portrait is unique to your energy signature.'
          )}
        </p>
            ...
            <div className="relative w-full max-w-md aspect-square rounded-lg overflow-hidden border-2 border-white/10 bg-black/20 flex items-center justify-center shadow-2xl">
              {loading ? (
                <div className="flex flex-col items-center gap-4">
                  ...
                  <span className="text-gold-200/80 animate-pulse">
                    {t('soul.generating', 'Summoning Ethereal Art...')}
                  </span>
                </div>
              ) : (
        ...
        >
          {loading
            ...
            : imageUrl
              ? t('soul.regenerate', 'Regenerate Portrait')
              : t('soul.generate', 'Reveal My Soul Portrait')}
        </Button>
      </Card>
    </div>
  );
