...
test('Security: Zodiac compatibility from /iching matches backend data', async ({ page }) => {
  const consoleErrors = [];
  ...
  await expect(page.getByRole('heading', { name: /I Ching Oracle/i })).toBeVisible();
  await page.screenshot({ path: buildScreenshotPath('security-zodiac-compat-iching-step-1') });
  ...
  await page.screenshot({ path: buildScreenshotPath('security-zodiac-compat-iching-step-2') });
