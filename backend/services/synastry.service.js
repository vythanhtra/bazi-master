...
export const calculateCompatibility = (chartA, chartB) => {
    ...
    if (dmRelation === 'Generates' || dmRelation === 'GeneratedBy') {
        score += 40;
        result.insights.push(`Strong emotional connection: The Day Masters support each other (${dmRelation}).`);
    } else if (dmRelation === 'Same') {
        score += 30;
        result.insights.push('Great friendship potential: Detailed understanding due to similar nature.');
